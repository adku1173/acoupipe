...
dataset.save_tfrecord(features=['sourcemap', 'loc', 'f'], f=2000, split='training', size=5, name=DATASET_NAME, progress_bar=False)
...
# %%
# The file is opened as a ``tf.data.TFRecordDataset`` and the parser is applied to
# each record. The first decoded sample is then retrieved.

tf_dataset = tf.data.TFRecordDataset(filenames=[DATASET_NAME]).map(parse_record)
sample = next(iter(tf_dataset))

# %%
# Finally, the decoded beamforming map of the first sample is displayed together with
...
# %%
# Writing the parser by hand shows what happens under the hood. For convenience, the
# dataset also provides a ready made parser via ``dataset.get_tfrecord_parser(...)``
# that returns an equivalent parser function.