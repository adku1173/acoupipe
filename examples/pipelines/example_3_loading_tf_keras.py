...
Once a dataset has been written to disk it can be streamed into a TensorFlow_ input pipeline for
training. This example loads a stored HDF5 dataset and turns it into a ``tf.data.Dataset`` that
a Keras model can consume, and then shows the analogous path for a dataset stored in the TFRecord
format.
"""
...
# %%
# A small dataset is written to an HDF5 file, here using the cross-spectral matrix (``csm``) as the
# input feature. Writing and loading datasets is covered in detail in the
...
# %%
# To build a ``tf.data.Dataset`` from the generator, TensorFlow_ needs to know the structure of the
# yielded samples, the so called output signature. If the shapes and dtypes are known, it can be
...
# %%
# Writing the signature by hand requires knowing every shape and dtype in advance. More
# conveniently, the dataset can provide a matching signature automatically
# via ``get_output_signature``.
...
# %%
# The resulting dataset yields dictionaries of tensors and can be iterated, batched and fed to a
# model like any other ``tf.data.Dataset``.
...
# %%
# The same pipeline can be built from a dataset stored in the TFRecord format. There, the parsing is
# handled by a parser function, which the dataset provides through ``get_tfrecord_parser``. See the
...
# %%
# From here either dataset can be batched and passed directly to the ``fit`` method of a Keras
# model, for example ``model.fit(tf_dataset.batch(8), ...)``.
