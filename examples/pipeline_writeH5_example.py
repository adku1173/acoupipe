...
# create Pipeline
pipeline = BasePipeline(sampler={1: rms_sampling}, numsamples=1, features=(extract_features, bb, wn))
...
metadata = {'sample_freq': 51200, 'freq': 1000, 'bandwidth': 'octave', 'block_size': 128}

# create TFRecordWriter to save pipeline output to TFRecord File
writer_sourcemap = WriteH5Dataset(
    source=pipeline,
...
writer_rms = WriteH5Dataset(source=writer_sourcemap, name='rms_' + DATASET_NAME, features=['rms'])

# #use the given save method
# writer_rms.save() # save data to file

# alternatively use the result method
for _d in writer_rms.get_data():
    pass
...
# create TFRecordWriter to save pipeline output to TFRecord File
writer_sourcemap = WriteTFRecord(
    source=pipeline,
    name='sourcemap_' + DATASET_NAME,
