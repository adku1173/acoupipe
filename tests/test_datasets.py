...
def test_values_correct(mode, feature, f, num, create_dataset):
    ...
    gen = dataset.generate(
        split="training", progress_bar=False, size=10000, start_idx=start_idx,
...
def test_save_h5(mode, feature, num, f, temp_dir, create_dataset):
    ...
    dataset.save_h5(split="training", size=2, features=[feature],
                    name=temp_dir / "test.h5", progress_bar=False)
...
@pytest.mark.parametrize("mode", modes)
@pytest.mark.parametrize("kwargs", [{"mic_pos_noise": True}, {"mic_pos_noise": False},
...
def test_sourcemap_max(mode, num, create_dataset):
    ...
    f = 4000
    dataset = create_dataset(full=False, mode=mode, mic_sig_noise=False, mic_pos_noise=False, max_nsources=1, snap_to_grid=True)
...
def test_eigvalsum_equal_csm(mode, create_dataset):
    ...
    num = 0
    f = 4000
    dataset = create_dataset(full=True, mode=mode, mic_sig_noise=False, mic_pos_noise=False, max_nsources=1, snap_to_grid=True)
    dataset.config.fft_params["block_size"] = 512
    gen = dataset.generate(
        f=f, num=num, features=features,
        split="training", size=1, progress_bar=False)
