...
class TestDatasetDummy:
            ...
            if hasattr(data[feature_name], 'shape'):
                ...
                for i, (expected, actual) in enumerate(zip(expected_shape, actual_shape)):
                    if expected is not None:
                        assert expected == actual, f"Shape mismatch for {feature_name}: expected {expected_shape}, got {actual_shape}"
    ...
    def test_feature_dtypes(self, dummy_dataset):
        ...
        for feature_name, expected_dtype in test_cases:
            gen = dummy_dataset.generate(
                features=[feature_name, 'idx', 'seeds'],
                size=1,
                split='training',
                f=1000,
                num=0,
            ...
            assert actual_dtype == expected_dtype, f"Dtype mismatch for {feature_name}: expected {expected_dtype}, got {actual_dtype}"
