...
def main(datasets,task_list,features,size,f,head,log):
    ray.init(head)
    ...
    for dataset in datasets:
        ...
        for tasks in task_list:
            for feature in features:
                ...
                # create dataset
                if feature == "None":
                    dataset = Dataset(features=[], f=f)
                else:
                    dataset = Dataset(features=[feature], f=f)

                gen = dataset.generate(split="training", tasks=tasks, size=initial_size+1, log=log, address=head, progress_bar=True)
                next(gen)
                ...
                if path.exists(filename):
                    ...
                    df = pd.DataFrame(
                        columns=["dataset","feature","f", "throughput", "size", "tasks", "time", "hostname", "head", "version"],
                        data=data)
                    df.to_pickle(filename)
    ray.shutdown()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasets", type=str, nargs="+", default=["dataset1", "dataset2"],)
    parser.add_argument("--task_list", type=int, nargs="+", default=[1,2,4,8,16,32])
    parser.add_argument("--features", type=str, nargs="+", default=["None","sourcemap","csmtriu","csm","eigmode"])
