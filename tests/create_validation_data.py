...
if __name__ == '__main__':
    ...
    if 'DatasetSynthetic' in args.dataset:
        for mode, f, num, feature in itertools.product(args.mode, frequencies, nums, args.features):
            ...
            data = next(
                dataset.generate(
                    f=f, num=num, features=[feature], split='training', progress_bar=False, size=1, start_idx=start_idx
                )
            )
            np.save(validation_data_path / f'DatasetSynthetic_{feature}_f{f}_num{num}_mode{mode}.npy', data[feature])
    ...
    if 'DatasetMIRACLE' in args.dataset:
        for mode, f, num, feature in itertools.product(args.mode, [1000], [0], args.features):
            ...
            np.save(validation_data_path / f'DatasetMIRACLE_{feature}_f{f}_num{num}_mode{mode}.npy', data[feature])
