...
def main(dataset, mode, split, size, features, f, num, start_idx, tasks, head, name, format, log):
    if log:
        import logging
    ...
    # save file
    if format == 'h5':
        Dataset(tasks=tasks, mode=mode).save_h5(
            features=features,
            f=f,
            num=num,
            split=split,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset',
        type=str,
        default='DatasetSynthetic',
        choices=['DatasetSynthetic', 'DatasetMIRACLE'],
    ...
    parser.add_argument(
        ...
        type=str,
        default='welch',
        choices=['welch', 'wishart', 'analytic'],
    ...
    parser.add_argument('--size', type=int, required=True, help='Total number of samples to simulate')
    parser.add_argument(
        '--start_idx',
        type=int,
        default=0,
    ...
    parser.add_argument('--log', action='store_true', help='Whether to log timing statistics. Only for internal use.')
