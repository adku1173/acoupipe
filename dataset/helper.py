...
def set_pipeline_seeds(pipeline,numsamples,dataset="training"):
    ...
    soff = int(1e7) # offset to ensure that seeds of sampler object never match
    pipeline.random_seeds = [range(off+(i*soff), off+(i*soff)+numsamples) for i in range(len(pipeline.sampler))]
