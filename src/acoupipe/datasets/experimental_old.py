...
from acoupipe.datasets.synthetic import FeatureCollectionBuilder as DatasetSyntheticFeatureCollectionBuilder
    ...
    def get_feature_collection(self, features, f, num):
        ...
        if "source_strength_analytic" in features:
            builder.add_source_strength_analytic(
                self.config.freq_data, f, num, ref_mic_index = self.config.ref_mic_index)
        if "source_strength_estimated" in features:
            if self.config.mode == "welch":
                builder.add_source_strength_estimated(
                self.config.fft_ref_spectra, f, num, ref_mic_index = self.config.ref_mic_index)
            else:
                builder.add_source_strength_estimated(
                    self.config.freq_data, f, num, ref_mic_index = self.config.ref_mic_index)
        ...
        if "targetmap_analytic" in features:
            builder.add_targetmap(self.config.freq_data, f, num, self.config.source_steer,
                ref_mic_index=self.config.ref_mic_index, strength_type="analytic", grid=self.config.grid)
        if "targetmap_estimated" in features:
            if self.config.mode == "welch":
                freq_data = self.config.fft_ref_spectra
            else:
                freq_data = self.config.freq_data
            builder.add_targetmap(freq_data, f, num, self.config.source_steer,
                ref_mic_index=self.config.ref_mic_index, strength_type="estimated", grid=self.config.grid)
        if "f" in features:
...
class MIRACLEFeatureCollectionBuilder(DatasetSyntheticFeatureCollectionBuilder):
    ...
    def add_source_strength_analytic(self, freq_data, f, num, ref_mic_index):
        from acoupipe.datasets.features import AnalyticSourceStrengthFeature
        calc_strength = AnalyticSourceStrengthFeature(
            freq_data=freq_data, f=f, num=num, ref_mic_index=ref_mic_index).feature_func
    ...
    def add_source_strength_estimated(self, freq_data, f, num, ref_mic_index) :
...
class DatasetMIRACLEConfig(DatasetSyntheticConfig):
    ...
    filename = Property()
    _filename = Str
    ref_mic_index = Either(Int(63), None, desc="reference microphone index (default: index of the centermost mic)")
    ...
    def create_acoular_pipeline(self):
        ...
        self.fft_ref_spectra = self.create_fft_ref_spectra()
    ...
    @staticmethod
    def calc_analytic_prepare_func(sampler, beamformer, filename, ref_mic_index):
        ...
        with h5.File(filename, "r", rdcc_nbytes=0) as file:
            ...
            for i in range(nsources):
                ...
                assert len(ir_idx) == 1
                if ref_mic_index is None:
                    tf = blockwise_transfer(
                        file["data/impulse_response"][ir_idx[0][0]], freq_data.block_size).T
                    transfer[:,:,i] = tf
                else:
                    transfer[:,:,i] = tf / tf[:,ref_mic_index][:,np.newaxis] # reference mic based normalization
            # adjust freq_data
    ...
    @staticmethod
    def calc_welch_prepare_func(sampler, beamformer, sources, fft_spectra, fft_ref_spectra, obs, filename, ref_mic_index):
        # we don't use a chunk cache here, since we access the data only once
        with h5.File(filename, "r", rdcc_nbytes=0) as file:
            ...
            for i,src in enumerate(subset_sources):
                ...
                if ref_mic_index is None:
                    ...
                    tf /= tf[:,ref_mic_index][:,np.newaxis] # reference mic based normalization
                # ifft to get kernel
            ...
            for src in obs_sources:
                src.mics = obs
                src.kernel = src.kernel[:,ref_mic_index][:,np.newaxis]
    ...
    def get_prepare_func(self):
        if self.mode == "welch":
            ...
            fft_ref_spectra=self.fft_ref_spectra,
