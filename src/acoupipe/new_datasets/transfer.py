...
from acoular.fastFuncs import _transferCoreFunc
...
class TransferMonopole(TransferBase):
    ...
    def transfer(self, f):
        ...
        -------
        array of complex128
            array of shape (num_freqs, ngridpts, nmics) containing the transfer matrix for the given frequency

        """
        # in cse it is a scalar, we need to convert it to an array
        f = np.array(f)
        wave_number = f * 2 * np.pi / self.env.c
        result = np.zeros(wave_number.shape + self.rm.shape, np.complex128)
        for i in range(wave_number.shape[0]):
            # transfer routine: parallelized over Gridpoints
            _transferCoreFunc(self.r0, self.rm, np.array([wave_number[i]]), result[i])
        return result
