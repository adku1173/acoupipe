...
from acoupipe import float_list_feature, PlanarSourceMapEvaluator,get_frequency_index_range
...
def get_sourcemap(beamformer, f=None, num=0, cache_dir=None, num_threads=1):
    ...
    if not f:
        ...
        return array([beamformer.synthetic(freq,num=num) for freq in f]) 
...
def _transform_csm(csm):
    ...
    return array(csm_list,dtype=float32)
...
def get_ref_mic_pow(source_mixer,power_spectra,fidx=None):
    ...
    p2 = array(p2)
    if fidx:
        return array([p2[:,indices[0]:indices[1]].sum(1) for indices in fidx]).T
    else:
        return p2
...
def _get_sourcemap_evaluator(beamformer,sourcemixer,powerspectra,f,num,r):
    if not f:
        ...
        target_p2 = get_ref_mic_pow(sourcemixer, powerspectra, fidx).T
        target_loc = array([array(s.loc) for s in sourcemixer.sources],dtype=float32)    
